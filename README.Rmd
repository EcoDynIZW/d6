...
2. Now run the function `new_project` to create a directory with all the scaffolding we use for all projects in our department. For example, to start a new project in the current working directory, just use:

```{r eval=FALSE}
new_project("project")
```

You don't need to change the working directory firstyou can also specify a path:

```{r eval=FALSE}
## both work:
new_project("project", path = "absolute/path/to/the/root/folder")
## or:
new_project("project", path = "absolute/path/to/the/root/folder/")
```
